...
const interpolateColor = (
    ...
    if (
        !/^#?[0-9A-Fa-f]{6}$/.test(firstHex) ||
        !/^#?[0-9A-Fa-f]{6}$/.test(secondHex) ||
        percent < 0 ||
        percent > 1
    ) {
    ...
    const interpolatedRgb = firstRgb.map((v, i) =>
        Math.round(v * (1 - percent) + secondRgb[i] * percent),
    );
    return (
        "#" +
        interpolatedRgb.map((v) => v.toString(16).padStart(2, "0")).join("")
    );
};
