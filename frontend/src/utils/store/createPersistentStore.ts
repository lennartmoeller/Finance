...
const createPersistentStore = <STATE, STATEDATA, SERIALIZED extends Serialized>(
    options: PersistentStoreOptions<STATE, STATEDATA, SERIALIZED>,
) => {
    const boundStore = create<STATE>()(
        persist(options.stateCreator, {
            name: options.name,
