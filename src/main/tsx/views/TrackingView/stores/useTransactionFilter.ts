...
type TransactionFilterState = TransactionFilterStateData & {
    setAccounts: (accounts: Array<number>) => void;
    setCategories: (categories: Array<number>) => void;
    setYearMonths: (yearMonths: Array<YearMonth>) => void;
}

const useTransactionFilter = () => {
    return createPersistentStore<TransactionFilterState, TransactionFilterStateData, TransactionFilterStateDataSerialized>({
        name: 'transactionFilter',
        stateCreator: (set) => ({
            accountIds: [],
            categoryIds: [],
            yearMonths: [YearMonth.fromDate(new Date())],
            setAccounts: (accountIds) => set({accountIds}),
