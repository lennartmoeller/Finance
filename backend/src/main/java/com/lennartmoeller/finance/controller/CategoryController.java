...
public class CategoryController {
    ...
    public ResponseEntity<CategoryDTO> getCategoryById(@PathVariable Long id) {
        return categoryService.findById(id).map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound()
                .build());
    }
    ...
    public CategoryDTO createOrUpdateCategory(@RequestBody CategoryDTO categoryDTO) {
        Optional<CategoryDTO> optionalCategoryDTO =
                Optional.ofNullable(categoryDTO.getId()).flatMap(categoryService::findById);
