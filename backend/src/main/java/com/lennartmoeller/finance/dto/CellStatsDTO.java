...
public class CellStatsDTO {
    ...
    public static CellStatsDTO add(List<CellStatsDTO> cellStatsDTOs) {
        ...
        result.setSurplus(StatsMetricDTO.add(
                cellStatsDTOs.stream().map(CellStatsDTO::getSurplus).toList()));
        result.setTarget(
                cellStatsDTOs.stream().mapToDouble(CellStatsDTO::getTarget).sum());
        result.setPerformance(PerformanceDTO.mean(
                cellStatsDTOs.stream().map(CellStatsDTO::getPerformance).toList()));
        return result;
    }
    ...
    public void calculatePerformance(
            ImmutableTriple<Double, Double, Double> rawBounds, ImmutableTriple<Double, Double, Double> smoothedBounds) {
        this.setPerformance(PerformanceDTO.calculate(
                this.getSurplus().getRaw(), rawBounds, this.getSurplus().getSmoothed(), smoothedBounds));
    }
}
